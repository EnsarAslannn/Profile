...
- **The first measurement is synchronous, not scheduled.** Going through `requestAnimationFrame` on mount put the callback behind the initial image decode: measured at 1440x900, the underline took **~1.3 seconds** to appear, so the bar sat blank and then lit up on its own. This is the one place the `react/set-state-in-effect` lint rule is disabled, with a reason - scroll position against layout is exactly the external system that rule exempts.
