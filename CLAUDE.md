...
**`ScrollToHash` sits out a search-only navigation, and that guard is a bug fix, not a refinement.** `LanguageProvider.setLanguage` switches language with `navigate({ pathname, search, hash }, { replace: true })` - and a **replace mints a fresh `location.key` exactly like a push does**, so this effect re-ran and a reader who had scrolled down to Projeler was thrown to the top of the page the moment they pressed EN. Comparing the previous `pathname` and `hash` against the current ones is what separates the two cases: a repeat click on the same section link changes neither and still scrolls (a navigation the reader asked for), while a language switch changes only the query string and is left alone. `ScrollToHash.test.tsx` pins both halves - the repeat-click case would pass with a naive "skip if the URL is unchanged" guard, which is why both are there.
...
| `dist/sitemap.xml`, `dist/<route>/index.html` | `scripts/build-seo-files.mjs` | every `npm run build` |
...
**Every internal `<Link>` carries the parameter through, via `useLocalizedTo()` (`src/i18n/useLocalizedTo.ts`).** They all used to drop it. That was invisible to a reader - `localStorage` kept the page English - but it cost the parameter both jobs it exists for: a URL copied out of the address bar mid-visit opened in Turkish for whoever it was sent to, and a crawler, which has no `localStorage` and no memory between requests, fell back to Turkish on the first click, leaving the entire English site uncrawlable past the landing page. It is carried **only when the current URL already has it**, so a Turkish reader still gets clean parameter-free links and nothing ever appends `?lang=tr` to a URL that means that already. For a string destination the parameter lands *before* the fragment: `/#projeler?lang=en` would make `projeler?lang=en` the fragment id, matching no section. `useLocalizedTo.test.tsx` reads the rendered hrefs rather than testing the hook, because the failure mode is a call site that forgot to use it - so **a new `<Link>` to an internal route must go through it**.
...
- `public/robots.txt` names the sitemap; `dist/sitemap.xml` is generated per build with the `xhtml:link` alternates that are how a crawler learns `?lang=en` exists. No `lastmod`/`changefreq`/`priority` - Google ignores the last two, and a `lastmod` stamped with the build time claims every page changed on every deploy.
