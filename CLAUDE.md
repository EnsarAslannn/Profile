...
**This last hop is unverified locally and needs one check on a preview deploy.** `vite preview` serves the prerendered file for `/hakkimda/` and `/hakkimda/index.html` but NOT for `/hakkimda` - its SPA fallback takes the extensionless path first - so a local check proves nothing about Vercel either way. Confirm with `curl -s https://<preview>/projects/dolfin | grep '<title>'`: `DOLFIN | Ensar Aslan` means the filesystem won, the site title means it did not. If it did not, the fix is explicit rewrites listed **before** the catch-all - and they must enumerate the real slugs, because `/projects/:slug -> /projects/:slug/index.html` would turn an unknown slug into a 404 instead of the redirect to `/` that `ProjectDetailPage` performs today.
