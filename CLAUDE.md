...
**The solid face is `bg-accent-soft text-accent-ink`** - the owner's CTA recipe, a muted sage fill carrying near-black ink at 4.91:1, not a saturated block of colour. **Its hover inverts rather than darkens**, and that is forced by the palette: `accent-hover` is a deep sage meant to be *read* as text on cream, so painting it behind `accent-ink` measures 2.24:1. Flipping to a deep-sage face with a cream label (`group-hover:bg-accent-base group-hover:text-surface-base`) measures 5.33:1. Both halves resolve through the tone scope, so the same two classes give a cream face with deep-green ink on a dark band - which is exactly what the owner specified for a button on a dark section, obtained for free rather than as a variant.
